...
void ArithmeticDecoding(std::string fn)
{
  ...
  {
    unsigned short int scale = probabilitiesHigh[256]; //nonadaptive model
    unsigned char c;
    ...
    for (n = 0; n < 257; ++n)
    {
      if (count >= probabilitiesLow[n] && count < probabilitiesHigh[n])
      {
        lowCount = probabilitiesLow[n];
        highCount = probabilitiesHigh[n];
        c = (unsigned char)n;
        break;
      }
    }
